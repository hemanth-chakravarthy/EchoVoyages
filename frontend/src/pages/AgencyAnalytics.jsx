...
const AgencyAnalytics = () => {
  ...
  const mockGuidePerformanceData = [
    { name: 'John Doe', ratings: 4.8, bookings: 45, revenue: 90000, satisfaction: 95, punctuality: 90, knowledge: 85 },
    { name: 'Jane Smith', ratings: 4.9, bookings: 52, revenue: 104000, satisfaction: 98, punctuality: 95, knowledge: 92 },
    { name: 'Mike Johnson', ratings: 4.6, bookings: 38, revenue: 76000, satisfaction: 92, punctuality: 88, knowledge: 90 },
    { name: 'Sarah Williams', ratings: 4.7, bookings: 42, revenue: 84000, satisfaction: 94, punctuality: 92, knowledge: 88 },
    { name: 'David Brown', ratings: 4.5, bookings: 35, revenue: 70000, satisfaction: 90, punctuality: 85, knowledge: 95 }
  ];

  const mockPackagePopularityData = [
    { name: 'Adventure in the Alps', bookings: 85, revenue: 170000, rating: 4.8 },
    ...
    { name: 'Greek Islands Cruise', bookings: 60, revenue: 120000, rating: 4.5 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch bookings data
        const bookingsResponse = await axios.get('http://localhost:5000/bookings', { headers });
        if (bookingsResponse.data && bookingsResponse.data.data) {
          ...
          const bookingsByMonth = Array(12).fill().map(() => ({ bookings: 0, revenue: 0 }));
        ...
        // Fetch guides data
        const guidesResponse = await axios.get('http://localhost:5000/guides', { headers });
        if (guidesResponse.data && guidesResponse.data.data) {
          ...
          const guidePerformanceData = guides.map(guide => {
            ...
            // Count bookings
            const bookingCount = guide.assignedPackages ? guide.assignedPackages.length : 0;
        ...
        // Fetch packages data
        const packagesResponse = await axios.get('http://localhost:5000/packages', { headers });
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">Agency Analytics Dashboard</h1>
        ...
        {isLoading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Seasonal Booking Trends</h2>
              {bookingData.length > 0 ? (
                ...
                <div className="h-80 flex items-center justify-center bg-gray-50 rounded-lg">
                  <div className="text-center p-6">
                    <p className="text-xl text-gray-500 mb-2">No booking data available</p>
                    <p className="text-sm text-gray-400">
                      Once you have bookings, you'll see trends and patterns here.
                    </p>
                  </div>
                </div>
              )}
              <p className="text-sm text-gray-500 mt-2">
                This chart shows booking trends and revenue throughout the year, helping you identify seasonal patterns.
              </p>
            </div>

            {/* Guide Performance Chart */}
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Guide Performance Comparison</h2>
              {guideData.length > 0 ? (
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="h-80 flex items-center justify-center bg-gray-50 rounded-lg">
                  <div className="text-center p-6">
                    <p className="text-xl text-gray-500 mb-2">No guide data available</p>
                    <p className="text-sm text-gray-400">
                      Once you have guides with ratings, you'll see performance comparisons here.
                    </p>
                  </div>
                </div>
              )}
              <p className="text-sm text-gray-500 mt-2">
                This radar chart compares your guides across different performance metrics, helping you identify strengths and areas for improvement.
              </p>
            </div>

            {/* Package Popularity Chart */}
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Package Popularity</h2>
              {packageData.length > 0 ? (
                ...
                <div className="h-80 flex items-center justify-center bg-gray-50 rounded-lg">
                  <div className="text-center p-6">
                    <p className="text-xl text-gray-500 mb-2">No package data available</p>
                    <p className="text-sm text-gray-400">
                      Once you have packages with bookings, you'll see popularity metrics here.
                    </p>
                  </div>
                </div>
              )}
              <p className="text-sm text-gray-500 mt-2">
                This chart shows your most popular packages based on booking count and ratings, helping you identify your best-performing offerings.
              </p>
            </div>
          </div>
        )}
