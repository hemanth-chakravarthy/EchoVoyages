...
const AgencyPayments = () => {
  ...
  useEffect(() => {
    const fetchGuides = async () => {
      ...
      try {
        ...
        const packagesResponse = await axios.get(`http://localhost:5000/packages/agents/${agencyId}`);
        ...
        const relevantGuides = guidesData.filter(guide => {
          if (!guide.assignedPackages || guide.assignedPackages.length === 0) return false;
  ...
  const handleGuideSelect = async (guideId) => {
    ...
    try {
      ...
      const guideResponse = await axios.get(`http://localhost:5000/guides/${guideId}`);
      const earningsResponse = await axios.get(`http://localhost:5000/guides/${guideId}/earnings`);
      ...
      const agencyRelatedPayments = allPayments.filter(payment => 
  ...
  const markAsPaid = async (guideId, earningId) => {
    try {
      setLoading(true);
      await axios.put(`http://localhost:5000/guides/${guideId}/earnings/${earningId}/paid`);
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold mb-8 text-[#1a365d]">Guide Payment Management</h1>
        ...
        <div className="card bg-white shadow-lg border border-gray-100 mb-8">
          <div className="card-body">
            ...
            {guides.length === 0 ? (
              <p className="text-gray-600">No guides are currently assigned to your packages.</p>
            ) : (
              <select 
        ...
        {selectedGuide && (
          ...
          >
            <div className="card-body">
              <h2 className="card-title text-2xl mb-6">Pending Payments for {selectedGuide.name}</h2>
                        ...
                        <tr key={payment._id} className="hover">
                          ...
                          <td>
                            <button 
                              ...
                              onClick={() => markAsPaid(selectedGuide._id, payment._id)}
                              disabled={loading}
                            >
