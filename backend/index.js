...
app.post('/forgot-password', (req, res) => {
  ...
  customers.findOne({ gmail: gmail })  // Make sure 'email' matches your schema field
    .then(user => {
      ...
      const token = jwt.sign({ id: user._id }, "Voyage_secret1", { expiresIn: '1h' });
  ...
  jwt.verify(token, "Voyage_secret1", (err, decoded) => {
    if (err) {
      ...
      bcrypt.hash(password, 12)
        .then(hash => {
          customers.findByIdAndUpdate(id, { password: hash })
            .then(() => res.json({ status: 'success' }))
            .catch(err => res.json({ status: 'Database error', error: err.message }));
        })
        .catch(err => res.json({ status: 'Error hashing password', error: err.message }));
    }
  });
});
